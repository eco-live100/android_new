...
class PharmacyListAdapter(var context: Context, var pharmacyList: ArrayList<PharmacyData>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding: RowPharmacyListBinding = DataBindingUtil.inflate(
            ...
            R.layout.row_pharmacy_list, parent, false)
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        "Timing : ${item.openingFrom}-${item.openingTo}".also { viewHolder.binding.servicesTv.text = it }
        viewHolder.binding.locationTv.text = item.location
        Glide.with(context).load(item.pharmacyImage).into(viewHolder.binding.chatImage)
       /* viewHolder.itemView.setOnClickListener {
            context.startActivity(
                Intent(context, PrescriptionRequestActivity::class.java)
                    .putExtra("doctorName","${item.pharmacyName}")
                    .putExtra(AppConstant.doctorId,"${item._id}")
            ...
            override fun performFiltering(charSequence: CharSequence): FilterResults {
                ...
                if (charSequence == null || charSequence.isEmpty()) {
                    ...
                    for (data in pharmacyList) {
                        ...
                        if (data.pharmacyName.lowercase(Locale.ROOT).contains(charSequence.toString().lowercase(Locale.ROOT))
